...
MergerPannel.Layout.prototype._initUi = function() {
	...
	$('#mergepannel-props-common select , #mergepannel-props-common input:checkbox').change(function() {
		realThis.applyProperties();
	});
}
/**
						...
						callback : {
							...
							onDrop : function(event, treeId, treeNodes,
								...
								for ( var i in treeNodes) {
									var eleView = document
											.getElementById(treeNodes[i].id);
									var eleTargetItem = document
											.getElementById(targetNode.id);

									if (moveType == 'prev') {
										realThis.moveBefore(eleView,
												eleTargetItem);
									} else if (moveType == 'next') {
										realThis.moveAfter(eleView,
												eleTargetItem);
									} else if (moveType == 'inner') {
							...
							onClick : function(event, treeId, treeNode) {
								...
								realThis.eleSelectedItem = document
										.getElementById(treeNode.id);
								realThis.dataSelectedItem = treeNode;
								realThis.openProperty(treeNode,
										realThis.eleSelectedItem);
							}
						}
					}, this.arrZTreeRootNodes);
...
MergerPannel.Layout.prototype.resizeDialog = function() {
	...
	$('#mergepannel-viewtree').height(
			$('#mergepannel-dialog').height()
					...
					- $('#mergepannel-layout-action').height() - 10);
...
MergerPannel.Layout.prototype.moveIn = function(view, frame) {
	...
	if ($(frame).children('.jc-layout-item-end').size() < 1) {
		...
		$(view)
				.insertBefore(
						$($(frame).children('.jc-layout-item-end').last()));
	}
	// frame 重新布局
...
MergerPannel.Layout.prototype.layoutFrame = function(frame, node) {
	...
	$(frame).removeClass('jc-frame-horizontal')
			.removeClass('jc-frame-vertical').children('.jc-layout')
			.removeClass('jc-layout-item-horizontal').removeClass(
					'jc-layout-item-vertical');
	// 设置样式
	var sLayout = node.layout;
	$(frame).addClass(MergerPannel.Layout.mapLayoutFrameStyles[sLayout])
			.children('.jc-layout').addClass(
					MergerPannel.Layout.mapLayoutItemStyles[sLayout]);
}
...
MergerPannel.Layout.prototype.openProperty = function(itemData, itemEle) {
	...
	if (itemData.type == 'frame') {
		...
		$('#mergepannel-props-frame-ipt-' + itemData.layout).attr('checked',
				true);
		console.log(itemData.id + '#mergepannel-props-frame-ipt-'
				+ itemData.layout);
...
MergerPannel.Layout.prototype.updateProperties = function() {
	...
	if ($(this.eleSelectedItem).hasClass('jc-view-skin-oc')) {
		$('#mergepannel-props-ipt-skin').attr('checked') = true;
	}
...
MergerPannel.Layout.prototype.applyProperties = function() {
	...
	if ($('#mergepannel-props-ipt-skin').get(0).checked == true) {
		$(this.eleSelectedItem).addClass('jc-view-skin-oc');
	}else{
		$(this.eleSelectedItem).removeClass('jc-view-skin-oc');
	}
	
	// this.eleSelectedItem.style.textAlign =
	// $('#mergepannel-props-sel-text-align').val() ;
	// this.eleSelectedItem.style.verticalAlign =
	// $('#mergepannel-props-sel-vertical-align').val() ;
}
/**
